...
# [Warning] This code is not suitable for PyTorch Tensors!!! (Only compatible with pure image files!!!)
...
class SSIM:
    ...
    def __call__(img1, img2):
        ...
        if img1.ndim == 2:                  # Gray (ndim == 2)
            ...
            if img1.shape[2] == 3:          # RGB  (ndim == 3)
                return np.array([SSIM._ssim(img1, img2) for _ in range(3)]).mean()
            elif img1.shape[2] == 1:        # Gray (ndim == 2)
